    ...
    if request.method == "POST":
        try:
            ...
            if image:
                        ...
                        if len(det):
                            ...
                            for *xyxy, conf, cls in reversed(det):
                                if save_img:  # Add bbox to image
                                    ...
                                    extra += "<div><b>" + str(names[int(cls)]) + "</b> (" + dict[names[int(cls)]] \
                                        + "): <b>{:.2f}% </b></div>".format(conf * 100)
                                print(extra)
                ...
                # Trả về kết quả
                if extra:
                    return render_template("index.html", xray_image=image.filename, rand=random.random(), msg="Tải file lên thành công", extra=Markup(extra))
                else:
                    return render_template("index.html", xray_image=image.filename, rand=random.random(), msg="Tải file lên thành công", extra=Markup('<div><b>Nothing found</b></div>'))
